...
exports.writeModelCode = writeModelCode;
const node_os_1 = require("node:os");
const defaultFields_js_1 = require("./defaultFields.js");
 ...
 */
async function writeModelCode(model, test) {
    let contents = templateHeader(model, test);
    if (test) {
        contents += 'import * as fr from "../../../src";' + node_os_1.EOL + node_os_1.EOL;
    }
    else {
        contents += 'import * as fr from "@azure/ai-form-recognizer";' + node_os_1.EOL + node_os_1.EOL;
    }
    const modelNameSlug = [
        ...model.modelId
            .split("-", 2)
    ...
    const modelName = (0, utils_js_1.camelCase)([...modelNameSlug, "Model"]);
    const documentTypeName = (0, utils_js_1.camelCase)([...modelNameSlug, "Document"]);
    const resultName = (0, utils_js_1.camelCase)([...modelNameSlug, "Result"]);
    const modelFeatures = (0, defaultFields_js_1.getFeatures)(model);
    const hasDocuments = modelFeatures.some((f) => f === "_documents");
    if (model.description) {
        contents += "/**" + node_os_1.EOL;
        contents +=
            model.description
    ...
    // add it later.
    contents += `export const ${modelName} = fr.createModelFromSchema(modelInfo()) as fr.DocumentModel<${resultName}>;${node_os_1.EOL + node_os_1.EOL}`;
    // Write the main Result interface.
    for (const line of writeResultInterface(resultName, documentTypeName, modelFeatures)) {
        contents += line + node_os_1.EOL;
    }
    contents += node_os_1.EOL;
    ...
    if (hasDocuments) {
        ...
        const documentType = variantNames.length > 1
            ? // In the case of multiple variants, a union
                variantNames.map((n) => `${node_os_1.EOL}  | ${n}`).join("")
        ...
        contents += `export type ${documentTypeName} =${documentType};${node_os_1.EOL + node_os_1.EOL}`;
    ...
    contents += "/**" + node_os_1.EOL;
    contents += " * The raw model schema." + node_os_1.EOL;
    contents += " */" + node_os_1.EOL;
    contents += "function modelInfo() {" + node_os_1.EOL;
    contents +=
        `  return ${JSON.stringify(model, null, 2)
            .split(/\r?\n/)
            .join(node_os_1.EOL + "  ")} as const;` + node_os_1.EOL;
    contents += "}" + node_os_1.EOL;
    return contents;
}
...
function extractModelVariants(model, _rootSlug) {
    ...
    for (const [docType, info] of Object.entries((_a = model.docTypes) !== null && _a !== void 0 ? _a : {})) {
        ...
        const docTypeName = (0, utils_js_1.camelCase)(slug);
        ...
        if (fieldEntries.length === 0) {
            ...
            yield* indent(flatMap(fieldEntries.map(([fieldName, schema]) => {
                ...
                return ({
                    ...
                    name: (0, utils_js_1.uncapitalize)(fieldName).replace(/\s/g, ""),
...
function writeType(schema, name, slug) {
    ...
    if (kind === "array") {
        ...
        const propertiesName = slug.map(utils_js_1.capitalize).join("") + name;
...
function* writeResultInterface(name, documentInterfaceName, features) {
    ...
    yield* indent(flatMap(features.filter((f) => f !== "_documents"), (f) => writeField(defaultFields_js_1.defaultResultFields[f])));
    if (hasDocuments) {
        yield* indent(writeField({
            name: "documents",
