...
const PDFViewer = ({ fileUrl, docTitle, docType, onClose, buyerPackageId }) => {
  const {
    ...
    zoomOut,
    handleDownload,
  } = PDFViewerLogic({ fileUrl, docTitle, docType, onClose, buyerPackageId });
    ...
    return Array.from({ length: numPages }, (_, i) => {
      ...
      return (
        <div
          ...
          className="fb-pdf-page-container"
        >
          {visiblePages.includes(pageNumber) ? (
            <Page
              ...
              loading={<div className="fb-pdf-page-loading">Loading page {pageNumber}...</div>}
              onRenderSuccess={() => {
            ...
            <div className="fb-pdf-page-placeholder" style={{ height: `${scale * 792}px` }}>
  ...
  return (
    ...
    >
      <div className="fb-pdf-viewer-header" onClick={(e) => e.stopPropagation()}>
        ...
        <button className="fb-pdfviewer-close-button" onClick={onClose}>
          <FiX />
        </button>
      </div>
      
      {error && (
        <div className="fb-pdf-error-message">
          {error}
        </div>
      )}
      
      <div 
        className="fb-pdf-viewer-container" 
        ref={containerRef}
      ...
      >
        {isLoading && (
          <div className="fb-pdf-spinner-overlay">
            <div className="fb-pdf-spinner"></div>
          </div>
        )}
      ...
      <div className={`fb-pdf-toolbar-container ${isToolbarVisible ? 'visible' : ''}`}>
        <div className="fb-pdf-toolbar">
