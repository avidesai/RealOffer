...
const BuyerPackageActivity = ({ buyerPackageId, listingId }) => {
  ...
  const fetchActivityStats = useCallback(async () => {
    try {
      ...
      if (!actualListingId) {
        console.error('No listing ID available for activity stats');
        setStatsLoaded(true); // Mark as loaded to prevent infinite loading
        return;
      }

      console.log('Fetching activity stats for listingId:', actualListingId);

      const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/activities/stats/${actualListingId}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  // If neither stats nor details are shown, display simplified view
  if (!showActivityStats && !showActivityDetails) {
    return (
      <div className="activity-tab">
        <SimplifiedActivityView metrics={metrics} />
      </div>
    );
  }
