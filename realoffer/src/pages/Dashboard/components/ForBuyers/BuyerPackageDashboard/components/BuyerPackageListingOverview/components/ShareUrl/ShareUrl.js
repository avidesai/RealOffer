...
const ShareUrl = ({ isOpen, onClose, url, listingId }) => {
  ...
  const handleSharePackage = async (e) => {
    ...
    try {
      // Use the listings share endpoint for both buyer packages and my listings
      const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/listings/share`, {
        listingId: listingId, // This will be the property listing ID
        shareUrl: url,
        recipient: {
          role: shareData.role,
      ...
      console.error('Error sharing buyer package:', error);
      setError(error.response?.data?.message || 'Failed to share buyer package. Please try again.');
    } finally {
      setIsSharing(false);
    }
  };
      ...
      <div className="share-url-content">
        ...
        {shareSuccess ? (
          <div className="share-url-success">
            ...
            <p>An email has been sent to {shareData.email} with access to this buyer package.</p>
          </div>
        ) : (
